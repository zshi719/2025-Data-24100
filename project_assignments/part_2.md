...
- `/api/v1/unique_stock_count`:
  - Return (in the body) a JSON object `{ 'unique_stock_count' : XXX }` where XXX is the number of unique stocks in the data.
- `/api/v1/row_by_market_count`:
  - Return (in the body) a JSON object `{ 'NYSE': xxx, 'NASDAQ': yyy }` where xxx and yyy represent the number of rows for each.
